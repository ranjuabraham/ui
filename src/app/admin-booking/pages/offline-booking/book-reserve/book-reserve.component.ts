...
export class BookReserveComponent implements OnInit, AfterViewInit {
  ...
  constructor(public fb: FormBuilder, private activatedRoute: ActivatedRoute, public appService: AppService,
    ...
    this.productDetailsForm = this.fb.group({
      ...
      foodId: [null],
      rentAmount: [null],
      editAdditionalHrAmt: [null],
      editDiscountAmt: [null],
      editSubTotalAmount: [null],
      editGstAmt: [null],
    });
    this.categoryId = 0;
    this.prdctDetId = this.loginData.prdctDetId;
    this.subHallId = 0;
  }
...
// convenience getter for easy access to form fields
  ...
  continueToBook() {
    ...
    this.http.post('/ema/venue/update-block', data).subscribe(res => {
      if (res['status'] === 'SUCCESS') {
        const hallName = 'the-checkers';
        const userId = JSON.parse(localStorage.getItem('usevenue-admin/offline/preview')).usrId;
        const bookingId = res['transaction'];
        const productDetId = res['prdctDetId'];
        this.router.navigate(['venue-admin/offline/preview'],
          {queryParams: {productDetId: productDetId, bookingId: bookingId}});
      } else {
        alert('Oops..., Something went wrong!');
      }
  ...
  continueToFoodMenu() {
    ...
    this.http.post('/ema/venue/update-block', this.collectData).subscribe(res => {
      if (res['status'] === 'SUCCESS') {
        this.router.navigate(['venue-admin/offline/food-menus'],
          {
            queryParams: {
  ...
  changeRentAmt() {
    this.editAdditionalHrAmt = this.tariffData['hourlyTar'] * this.count;
    const beforeDiscount = this.editAdditionalHrAmt + (+this.editRentAmt);
    this.editSubTotalAmount = beforeDiscount - this.discountAmount;
    this.editGstAmt = this.editSubTotalAmount * this.gstValue / 100;
    this.totalAmount = this.editGstAmt + this.editSubTotalAmount;
  }

  changeDiscountAmt() {
    this.editAdditionalHrAmt = this.tariffData['hourlyTar'] * this.count;
    const beforeDiscount = (this.editAdditionalHrAmt * this.count) + (+this.editRentAmt);
    this.editSubTotalAmount = beforeDiscount - this.editDiscountAmt;
    this.editGstAmt = this.editSubTotalAmount * this.gstValue / 100;
    this.totalAmount = this.editGstAmt + this.editSubTotalAmount;
  }
  ...
  continueToEditBook() {
    const tariffData = {
      rentAmount: this.editRentAmt,
      discountValue: this.discountValue,
      discountAmount: this.editDiscountAmt,
      hourAmount: this.hourAmount,
      additionalHours: this.count,
      additionalHoursAmount: this.additionalHoursAmount,
      gstAmount: this.editGstAmt,
      gstValue: this.gstValue,
