...
@Component({
  selector: 'app-hall-details-edit',
  templateUrl: './hall-details-edit.component.html',
  styleUrls: ['./hall-details-edit.component.scss']
})
export class HallDetailsEditComponent implements OnInit {
  ...
  ngOnInit() {
    this.hallInfoService.loadItemById(this.$prdctDetId, this.$attrId)
    .subscribe(response => {  
      this.resultInfo = response;
      this.enableFlag('hourly', this.resultInfo.hourlyFlg);
      this.enableFlag('buffet', this.resultInfo.buffetFlg);
      this.enableFlag('discount', this.resultInfo.disocuntFlg);
      });
  }
