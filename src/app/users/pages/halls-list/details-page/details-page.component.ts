import {AfterViewInit, Component, OnInit,Inject, ElementRef, HostListener} from '@angular/core';
...
export class DetailsPageComponent implements OnInit, AfterViewInit {
  ...
  myDatePickerSearch:IMyDpOptions;
  ...
  constructor(public fb: FormBuilder, private bottomSheet: MatBottomSheet,private activatedRoute: ActivatedRoute, public appService: AppService,
    ...
    this.prdctDetId = this.route.snapshot.queryParamMap.get('venueId');
    this.subHallId = this.route.snapshot.queryParamMap.get('hallId');
  }
  ...
  getProductDetails() {
    this.appService.getProductDetails(this.prdctDetId, this.subHallId, this.date,
      ...
      .subscribe(type => {
        ...
        this.pricePerPlate = this.productDetails.tariff['buffet'][0];
        // tariff calculations
        ...
        this.myDatePickerOptions = {
          ...
          inline: false,
          markDates: this.highLightsDate,
          disableDays: this.blockedDate.dates,
          allowSelectionOnlyInCurrentMonth: false,
          showInputField: true,
          markCurrentDay: true,
          indicateInvalidDate: true,
...
// convenience getter for easy access to form fields
  ...
  amentiesBottomsheet(){
    this.bottomSheet.open(BottomSheetAmeneties, {
      data: {
        amenitiesData: this.amenitiesData,
      },
      panelClass: 'bottom-sheet-detail-page-sheet'
    });
  }
  continueToFoodMenu() {
    console.log(this.pricePerPlate['buffMin']);
    console.log('krishna moorthi');
    const data = Object.assign(this.collectionData(), {
      blockType: 1, // book or reserve
      tariffData: [],
      buffetData: '',
      guaranteedPax: this.pricePerPlate['buffMin']
    });
...
@Component({
  selector: 'bottom-sheet-detail-page-sheet',
  templateUrl: 'bottom-sheet-detail-page.html',
  styleUrls: ['./details-page.component.scss']
})
export class BottomSheetAmeneties implements OnInit {
  constructor(private bottomSheetRef: MatBottomSheetRef<BottomSheetAmeneties>,        
          @Inject(MAT_BOTTOM_SHEET_DATA) public data: any) {}

      amenitiesData;

      ngOnInit(): void {
            this.amenitiesData = this.data['amenitiesData'];
