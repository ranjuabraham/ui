...
export class MyDatePickerComponent implements OnChanges, ControlValueAccessor, OnDestroy {
  ...
  @Output() userDateInput: EventEmitter<any> = new EventEmitter<any>();
  ...
  opts: IMyOptions = {
    ...
    material: <boolean>false, // custom krishna
    auspiciousDayOnly: <boolean>false, // custom krishna
  ...
  onTouchedCb: () => void = () => {}
  constructor(public elem: ElementRef, private renderer: Renderer, private cdr: ChangeDetectorRef,
              private localeService: LocaleService, private utilService: UtilService) {
  ...
  onUserDateInput(value: string): void {
    if (value.length === 0) {
      ...
      const date: IMyDate = this.utilService.isDateValid(value, this.opts.dateFormat,
        ...
        this.opts.disableWeekends, this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges,
        this.opts.monthLabels, this.opts.enableDays);
  ...
  isTodayDisabled(): void {
    this.disableTodayBtn = this.utilService.isDisabledDay(this.getToday(), this.opts.minYear,
      this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends,
      this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.enableDays);
  }
  ...
  writeValue(value: any): void {
    if (value && (value['date'] || value['jsdate'] || value['formatted'])) {
      this.selectedDate = value['date'] ? this.parseSelectedDate(value['date']) : value['jsdate'] ? this.parseSelectedDate
      (this.jsDateToMyDate(value['jsdate'])) : this.parseSelectedDate(value['formatted']);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.hasOwnProperty('selDate')) {
      ...
      if (sd.currentValue !== null
        && sd.currentValue !== undefined && sd.currentValue !== '' && Object.keys(sd.currentValue).length !== 0) {
  ...
  openSelector(reason: number): void {
    this.globalListener = this.globalListener || this.renderer.listenGlobal('document', 'click', (event: any) => {
      if (this.showSelector && event.target && this.elem.nativeElement !== event.target
  ...
  onCellClicked(cell: any): void {
    ...
    this.userDateInput.emit(cell.dateObj);
      ...
      if (i === 1) {
        ...
        for (let j = pm; j <= dInPrevM; j++) {
          ...
          week.push({
            ...
            disabled: this.utilService.isDisabledDay(date, this.opts.minYear,
              this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends,
              this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.enableDays),
        ...
        for (let j = 0; j < daysLeft; j++) {
          ...
          week.push({
            ...
            disabled: this.utilService.isDisabledDay(date, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil,
              this.opts.disableSince, this.opts.disableWeekends, this.opts.disableWeekdays,
              this.opts.disableDays, this.opts.disableDateRanges, this.opts.enableDays),
            markedDate: this.utilService.isMarkedDate(date, this.opts.markDates, this.opts.markWeekends),
            highlight: this.utilService.isHighlightedDate(date, this.opts.sunHighlight, this.opts.satHighlight, this.opts.highlightDates)
          });
          dayNbr++;
        ...
        for (let j = 1; j < 8; j++) {
          ...
          week.push({
            dateObj: date, cmo: cmo, currDay: this.isCurrDay(dayNbr, m, y, cmo, today),
            disabled: this.utilService.isDisabledDay(date, this.opts.minYear, this.opts.maxYear,
  ...
  parseSelectedDate(selDate: any): IMyDate {
    ...
    if (typeof selDate === 'string') {
      ...
      date.month = df.indexOf(MMM) !== -1 ? this.utilService.getMonthNumberByMonthName(dateValue[1],
        this.opts.monthLabels) : this.utilService.getNumberByValue(dateValue[1]);
